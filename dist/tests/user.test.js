...
jest.mock('bcrypt', () => ({
    ...
    compare: jest.fn().mockImplementation(async (pwd, hashed) => hashed === `hashed:${pwd}`),
}));
        ...
        getConnection: async () => ({
            ...
            query: async (sql, params = []) => {
                ...
                if (/^SELECT \* FROM users WHERE email = \?/.test(sql)) {
                    if (mem.forceLoginServerError)
                        throw new Error('forced-login-error');
...
describe('User Controller', () => {
    ...
    it('deve listar todos os usuários (200)', async () => {
        // cria 1
        await (0, supertest_1.default)(index_1.default).post('/users').send({
            name: 'A',
            email: 'a@example.com',
            cpf: '111.111.111-11',
            password: 'pw',
        ...
        expect(res.body.data[0]).toEqual(expect.objectContaining({ id: expect.any(Number), name: 'A', email: 'a@example.com', cpf: '111.111.111-11' }));
    ...
    it('deve atualizar um usuário (200)', async () => {
        const created = await (0, supertest_1.default)(index_1.default).post('/users').send({
            name: 'Old',
    ...
    it('deve retornar 409 se email/CPF já cadastrado por outro no update', async () => {
        // cria 2 usuários
        const a = await (0, supertest_1.default)(index_1.default).post('/users').send({
            name: 'A',
            email: 'a@example.com',
            cpf: '111.111.111-11',
            password: 'pw',
        });
        const b = await (0, supertest_1.default)(index_1.default).post('/users').send({
            name: 'B',
            email: 'b@example.com',
            cpf: '222.222.222-22',
            password: 'pw',
    ...
    it('deve deletar usuário (200)', async () => {
        ...
        const id = created.body.data.id;
        const res = await (0, supertest_1.default)(index_1.default).delete(`/users/${id}`).set(auth());
        expect(res.status).toBe(200);
        expect(res.body.status).toBe('success');
    });
    it('deve retornar 404 ao deletar usuário inexistente', async () => {
        const res = await (0, supertest_1.default)(index_1.default).delete('/users/9999').set(auth());
        expect(res.status).toBe(404);
        expect(res.body.status).toBe('error');
    });
