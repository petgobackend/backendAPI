...
// Em teste unitário não queremos testar JWT de verdade
// então só garantimos que req.userId seja igual ao id da rota
...
// 3) Mock do banco de dados em memória
    ...
    getConnection: async () => ({
      ...
      query: async (sql: string, params: any[] = []) => {
        ...
        if (/^UPDATE users SET/.test(sql)) {
          ...
          // Suporta os dois formatos de SQL:
          ...
          if (/name = \?, email = \?, cpf = \? WHERE id = \?/.test(sql)) {
            ;[name, email, cpf, idParam] = params
          } else if (/name = \?, cpf = \? WHERE id = \?/.test(sql)) {
            ;[name, cpf, idParam] = params
            // mantém o email antigo
            const existing = mem.users.find(u => u.id === Number(idParam))
...
// Cabeçalho de auth para rotas protegidas
...
describe('User Controller', () => {
  ...
  it('deve retornar 409 se email ou CPF já cadastrado (ER_DUP_ENTRY)', async () => {
  ...
  it('deve retornar 409 se CPF já cadastrado por outro no update', async () => {
  ...
  it('deve retornar 400 no login se faltar email ou senha', async () => {
    const res = await request(app).post('/users/login').send({
      email: 'x@example.com'
    })
